...
@section('content')
  ...
  <div class="card mt-5">
    ...
    <div class="card-body table-responsive">
      <form action="{{ route('admin.common_questions.store') }}" method="post" enctype="multipart/form-data">@csrf
        <div class="row">
          @foreach ($locales as $locale)
            <div class="form-group col-6">
              <label>@lang("question_$locale")</label>
              <textarea class="form-control" name={{ $locale }}[question]
                        rows="10">{{ old("$locale.question") }}</textarea>
            </div>
          @endforeach
          @foreach ($locales as $locale)
            <div class="form-group col-6">
              ...
              <textarea class="form-control" name={{ $locale }}[answer]
                        rows="10">{{ old("$locale.answer") }}</textarea>
            </div>
          @endforeach
          <div class="form-group col-6">
